...
int main()
{
	...
	try {
		...
		inet_pton(AF_INET, "192.168.43.153", &(serv.sin_addr)); // адрес сервера
...
string GetErrorMsgText(int code) // Функция позволяет получить сообщение ошибки
{
	...
	case WSAECONNRESET:	return "WSAECONNRESET: Существующее соединение было принудительно закрыто удаленным хостом.";
