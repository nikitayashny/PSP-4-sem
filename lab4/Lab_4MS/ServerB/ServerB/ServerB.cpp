...
//для подтверждения сервером запроса клиента на установку соединения 
//(Функция оправляет в адрес клиента (параметры сокета клиента указываются в параметре to)
//свой позывной, что предполагает готовность сервера  к дальнейшей работе с клиентом)
...
int main() {
	...
	try {
		...
		while (true) {
			...
			//исправил, чтобы отвечал клиенту
