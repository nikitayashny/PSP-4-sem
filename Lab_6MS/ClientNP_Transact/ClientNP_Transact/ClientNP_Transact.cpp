...
#define PIPEG TEXT("\\\\TarakanovNS\\pipe\\Tube")
...
int main()
{
	try {
		...
		if ((cH = CreateFile(PIPEG, // Имя канала
			...
			0, // нету совместного чтения и записи
		...
		SetNamedPipeHandleState(cH, &state, NULL, NULL);//функция предназначена для изменения динамических характеристик именованного канала
